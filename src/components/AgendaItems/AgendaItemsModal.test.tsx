...
} from '../../screens/OrganizationAgendaItems/OrganizationAgendaItemsErrorMocks';
...
describe('Testing Check In Attendees Modal', () => {
  ...
  test('The modal should be rendered properly', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText('Event Agenda Items')).toBeInTheDocument(),
    );

    await waitFor(() => {
      return expect(
        screen.findByTestId('createEventAgendaItemBtn'),
  ...
  test('render error component on unsuccessful agenda item category list query', async () => {
    ...
    await waitFor(() => {
      expect(queryByText('createEventAgendaItemBtn')).not.toBeInTheDocument();
    });
  });
  ...
  test('render error component on unsuccessful agenda items list query', async () => {
    const { queryByText } = render(
  ...
  test('creates new agenda item associated with the event', async () => {
    render(
      <MockedProvider addTypename={false} link={link}>
        <Provider store={store}>
          <BrowserRouter>
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId('createEventAgendaItemBtn'),
      ).toBeInTheDocument();
    });
    userEvent.click(screen.getByTestId('createEventAgendaItemBtn'));
